...
class RINBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.latents_attend_to_patches = AttentionBlock(latent_width, dim_context = latent_width, norm = True, norm_context = True, **attn_kwargs)
        ...
        self.patches_peg = PositionalEncodingGenerator(latent_width)
        self.patches_self_attn = LinearAttention(latent_width, norm=True, **attn_kwargs)
        self.patches_self_attn_ff = FeedForward(latent_width)
        ...
        self.patches_cross_attn_ff = FeedForward(latent_width)

    def forward(self, patches: Tensor, latents, t) -> Tensor:
